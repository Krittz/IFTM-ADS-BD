...
WHERE
    h.evento LIKE "%Segun%"
    AND d.checkout IS NULL;

-- #D

SELECT
    TIMEDIFF(hora_final, hora_inicio) as Horas,
    s.local as Sala
from atividades a
    JOIN salas s ON s.id = a.sala_id
GROUP BY s.id;
...
SELECT
    ...
    TIMEDIFF(hora_final, hora_inicio) as Horas
from turmas t
    JOIN participantes_turmas pt ON pt.turma_id = t.id
    JOIN participantes p ON p.id = pt.participante_id
    JOIN participantes_atividades pa ON pa.participante_id = p.id
    JOIN atividades a ON a.id = pa.atividade_id
    JOIN turnos tr ON tr.id = a.turno_id
WHERE
    tr.periodo = 'Matutino'
    AND a.data = '2018-10-04'
GROUP BY t.id;